...
class Terrain:
    def __init__(self, json):
        ...
        # Masking layer
        self.m_masks = []
        for r in range(len(json['masks'])):
            row = []
            for c in range(len(json['masks'][r])):
                col = json['masks'][r][c]
                row.append(col)
            self.m_masks.append(row)
    ...
    def render_masking_layer(self, window):
        for row in range(len(self.m_masks)):
            for col in range(len(self.m_masks[row])):
                if self.m_masks[row][col][0] != MyDefine.INVALID_ID:
                    res = ImageManager.get_instance().find_resource_by_name(self.m_files[self.m_masks[row][col][0]])
                    window.blit(res["image"], (col * MyDefine.TILE_RESOLUTION[0], row * MyDefine.TILE_RESOLUTION[1]),
                                (self.m_masks[row][col][1][1] * MyDefine.TILE_RESOLUTION[0],
                                 self.m_masks[row][col][1][0] * MyDefine.TILE_RESOLUTION[1],
                                 MyDefine.TILE_RESOLUTION[0],
                                 MyDefine.TILE_RESOLUTION[1]))
